...
/**
 * spark worker对象
 * akka生命周期：preStart receive stop
 *
 * @param sparkWorkerHostname
...
object SparkWorker {
  def main(args: Array[String]): Unit = {
    ...
    // TODO_MA 注释： 这句代码执行的时候，会创建 SparkWorker 的 actor 对象
    // TODO_MA 注释： 同时再跳转到这个 actor 的声明周期方法： preStart()
    // TODO_MA 注释： 每个actor的内部有三个声明周期方法： preStart(), receive(), onStop()
